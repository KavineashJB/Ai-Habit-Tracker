...
export default function OrbitingHabits() {
  ...
  const sunGradient = isDark
    ? "radial-gradient(circle at 30% 25%, rgba(56,189,248,0.4), rgba(14,165,233,0.24) 60%, rgba(3,105,161,0.2))"
    : "radial-gradient(circle at 30% 25%, #ffffff, #e0f2fe 45%, #bae6fd 100%)";

  const sunShadow = isDark
    ? "inset 0 1px 0 rgba(255,255,255,0.35), 0 6px 24px rgba(14,165,233,0.3)"
    : "inset 0 1px 0 rgba(255,255,255,0.9), 0 8px 24px rgba(14,165,233,0.22), 0 2px 6px rgba(14,165,233,0.14)";

  const sunBorder = isDark
    ? "1px solid rgba(255,255,255,0.2)"
    : "1px solid rgba(255,255,255,0.8)";

  const sunIconColor = isDark ? "#7dd3fc" : "#0369a1";

  const haloBg = isDark
    ? "radial-gradient(circle, rgba(14,165,233,0.24), transparent 70%)"
  ...
  return (
    <div className="relative mx-auto w-[300px] h-[300px] sm:w-[400px] sm:h-[400px] lg:w-[460px] lg:h-[460px]">
      ...
      {STARS.map((s, i) => (
        <span
          key={i}
          className="absolute rounded-full bg-sky-400/70 dark:bg-sky-200/70"
          style={{
            top: s.top,
            left: s.left,
            width: s.size,
